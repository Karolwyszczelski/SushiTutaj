...
const START_HOUR = 11;
const START_MIN = 30;
const END_HOUR = 22;     // do 22:00
const MAX_PER_SLOT = 5;

// Opcjonalna granica przyjmowania zgłoszeń (np. 21:45)
...
/** wygeneruj dozwolone sloty w danym dniu (wycina przeszłe dla "dziś") */
function generateSlots(dateStr: string) {
  const selected = new Date(`${dateStr}T00:00:00`);
  const slots: string[] = [];
  const d = new Date(selected);
  d.setHours(START_HOUR, START_MIN, 0, 0);
  while (d.getHours() < END_HOUR || (d.getHours() === END_HOUR && d.getMinutes() === 0)) {
  ...
  try {
    ...
    if (!rid && restaurant_slug) {
      const { data: r, error: rErr } = await supabaseAdmin
        ...
        .maybeSingle();
      if (rErr) {
        return NextResponse.json({ error: "Błąd sprawdzania restauracji." }, { status: 500 });
      }
      rid = r?.id ?? null;
    }
    if (!rid) {
      return NextResponse.json({ error: "Nie wykryto restauracji." }, { status: 400 });
    }
    ...
    if (!allowed.includes(timeHHMM)) {
      return NextResponse.json({ error: "Wybrany termin jest niedostępny." }, { status: 409 });
    }

    // Opcjonalnie zamknij przyjmowanie poza oknem (zostawiamy jak w poprzednim kodzie)
    ...
    if ((count ?? 0) >= MAX_PER_SLOT) {
      return NextResponse.json({ error: "Wybrana godzina jest już pełna." }, { status: 409 });
    }

    // Zapis rezerwacji w naszej docelowej strukturze
    const insertPayload = {
      restaurant_id: rid,
      reservation_date: day,           // date
      reservation_time: timeHHMMSS,    // time
      guests,                          // int
      name,                            // text
      phone,                           // text
      email,                           // text (wymagany)
      note,                            // text
      status: "new" as const,          // new | accepted | cancelled
    ...
    // (opcjonalnie) tu można dodać wysyłkę e-mail potwierdzającego przyjęcie zgłoszenia „new”
    // Potwierdzenie akceptacji wyśle dedykowany endpoint /api/reservations/update po akcji w panelu.
