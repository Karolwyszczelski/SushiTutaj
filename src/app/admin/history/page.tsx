  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const slug = (j?.restaurant_slug as string | undefined) || getCookie("restaurant_slug") || undefined;
        if (slug && CITY_OPTIONS.some((c) => c.slug === slug)) setCity(slug as CityPick);
      } finally {
        setBooted(true);
      }
    };
  ...
  const filtered = useMemo(() => {
    ...
    return orders
      ...
      .filter((o) => {
        ...
        const optHit = String((o as any).selected_option || "").toLowerCase().includes(term);
        return idHit || nameHit || optHit;
      });
  }, [orders, statusFilter, search]);

  const counts = useMemo(() => {
    const s = (k: string) => orders.filter((o) => ((o.status || "").toString().toLowerCase() === k)).length;
    return {
      all: orders.length,
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">Przegląd, filtry i eksport.</p>
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`rounded-full px-3 py-1.5 text-sm font-medium shadow-sm ${
                statusFilter === s.key ? "bg-slate-900 text-white" : "bg-white text-slate-700 hover:bg-slate-100 border"
              }`}
            >
              {s.label} <span className="ml-1 rounded-full bg-slate-200 px-2 py-0.5 text-xs">{s.count}</span>
            </button>
          ))}
        </div>
          ...
          <table className="min-w-full table-auto">
            <thead className="bg-slate-50 text-left text-slate-600">
              <tr>
                ...
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wide">Data</th>
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wide">Klient</th>
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wide">Kwota</th>
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wide">Opcja</th>
              ...
              {filtered.map((o, i) => {
                ...
                return (
                  <tr key={o.id || i} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-700">{optionLabel((o as any).selected_option)}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-block rounded-full px-2.5 py-1 text-xs font-semibold ${statusBadge(s)}`}>
                        {statusLabel(s)}
                      </span>
                    </td>
                  </tr>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <button
          onClick={() => router.push(city === "all" ? "/admin/pickup-order" : `/admin/pickup-order?restaurant=${city}`)}
        ...
        <button
          onClick={() => {
            // prosty eksport CSV z aktualnego widoku
            const rows = [
