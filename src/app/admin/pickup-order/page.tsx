...
const fromDBPaymentStatus = (v: any): PaymentStatus => {
  ...
  if (["pending", "processing", "inprogress", "authorized", "auth"].includes(s)) return "pending";
  return null;
};

const getOptionLabel = (opt?: Order["selected_option"]) =>
  opt === "delivery" ? "DOSTAWA" : opt === "takeaway" ? "NA WYNOS" : "BRAK";
...
const collectStrings = (val: any): string[] => {
  ...
  if (typeof val === "object") {
    const truthy = Object.entries(val).filter(([, v]) => v === true || v === 1 || v === "1").map(([k]) => k);
...
const AcceptButton: React.FC<{
  ...
  const options: number[] = useMemo(
    () => (order.selected_option === "delivery" ? [30, 60, 90, 120] : [15, 20, 30, 45, 60]),
        ...
        <div className="absolute left-0 top-11 z-10 w-48 overflow-hidden rounded-md border bg-white text-slate-900 shadow-lg">
          {options.map((m) => (
            <button
              key={m}
              type="button"
              onClick={() => { setMinutes(m); setOpen(false); onAccept(m); }}
...
const extractChopsticksFromOrderRaw = (o: any): number | null => {
  ...
  const top = readNestedInt(o, [
    "chopsticks_count", "chopsticks", "paleczki", "paleczki_count", "sticks", "ilosc_paleczek", "ilosc_pałeczek",
  ]);
  if (top !== null) return top;

  // poszukaj w meta / options / data
  const deepCandidates = [
    o?.meta, o?.options, o?.data, o?.extra, o?.details, o?.legal_accept, o?.summary,
  ].filter(Boolean);
  ...
  try {
    ...
    if (items && typeof items === "object") {
      const n =
        ...
        (Array.isArray(items)
          ? items.reduce<number | null>((acc, it) => acc ?? extractChopsticksFromOrderRaw(it), null)
          : null);
      if (n !== null) return n;
    }
  } catch {}
...
export default function PickupOrdersPage() {
  ...
  useEffect(() => {
    const url = urlSlug
      ? `/api/restaurants/ensure-cookie?restaurant=${encodeURIComponent(urlSlug)}`
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/orders/current?${qs.toString()}`, { cache: "no-store", signal: ac.signal });
  ...
  }, [booted, page, perPage, sortOrder, editingOrderId, playDing, restaurantSlug, urlSlug]);
  ...
  const refreshPaymentStatus = async (id: string) => {
    try {
      ...
      setOrders((prev) => prev.map((o) => (o.id === id ? { ...o, payment_status: fromDBPaymentStatus(payment_status) } : o)));
    } finally {
  ...
  const acceptAndSetTime = async (order: Order, minutes: number) => {
    try {
      ...
      const r = await fetch(`/api/orders/${order.id}/accept`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      updateLocal(order.id, { status: "accepted", deliveryTime: j.deliveryTime as string });
    } finally {
      setEditingOrderId(null);
    }
  };
  ...
  const newList = filtered.filter((o) => o.status === "new" || o.status === "pending" || o.status === "placed");
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            {p.ingredients.length > 0 && (
              <div className="mt-0.5 text-[12px] text-slate-700">Skład: {p.ingredients.join(", ")}</div>
            )}
            {p.note && <div className="mt-0.5 text-[12px] italic text-slate-800">Notatka: {p.note}</div>}
            {onDetails && (
      ...
      <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-4 sm:items-center">
        <div className="w-full max-w-lg rounded-md border bg-white p-5 text-slate-900 shadow-2xl">
          ...
          <div className="space-y-2 text-sm">
            ...
            {p.note && <div className="italic text-slate-800">{p.note}</div>}
          </div>
        </div>
      </div>
    );
  ...
  const OrderCard: React.FC<{ o: Order }> = ({ o }) => {
    ...
    return (
      <article
        key={o.id}
        className={`rounded-2xl border bg-white p-5 shadow-sm ring-1 ${statusTone(o.status)} text-slate-900`}
      >
        <header className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="text-sm text-slate-700">
              ...
              <span className="ml-3">
                ...
                {o.clientDelivery === "asap"
                  ? "Jak najszybciej"
                  : o.clientDelivery
                    ? new Date(o.clientDelivery).toLocaleTimeString("pl-PL", { hour: "2-digit", minute: "2-digit" })
                    : "-"}
              </span>
            </div>
          </div>
          <div className="flex items-center gap-3 text-sm">
            ...
            <span className="text-slate-600">{new Date(o.created_at).toLocaleString("pl-PL")}</span>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="space-y-2 text-sm text-slate-800">
            ...
            {/* NEW: Pałeczki */}
            <div className="mt-3">
              <label className="block text-sm font-semibold text-slate-800 mb-1">Pałeczki</label>
              <div className="inline-flex items-center gap-2">
                <button
                  onClick={() => setChopsticks(o, Math.max(0, (o.chopsticks ?? 0) - 1))}
                ...
                />
                <button
                  onClick={() => setChopsticks(o, (o.chopsticks ?? 0) + 1)}
                  className="h-8 w-8 rounded-md border bg-white text-slate-900 hover:bg-slate-50"
                  disabled={editingOrderId === o.id}
          ...
          <div className="sm:col-span-2">
            <div className="mb-1 text-sm font-semibold text-slate-800">Produkty</div>
            {prods.length === 0 ? (
  ...
  return (
    <div className="mx-auto max-w-6xl p-4 text-slate-900 sm:p-6">
      ...
      <div className="sticky top-0 z-20 -mx-4 mb-5 bg-white p-4 text-slate-900 sm:mx-0 sm:rounded-2xl sm:border">
        <div className="flex flex-wrap items-center gap-2">
          <select className="h-10 rounded-md border bg-white px-3 text-sm text-slate-900" value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as any)}>
          ...
          <select className="h-10 rounded-md border bg-white px-3 text-sm text-slate-900" value={filterOption} onChange={(e) => setFilterOption(e.target.value as any)}>
            <option value="all">Wszystkie opcje</option>
            <option value="takeaway">Na wynos</option>
            <option value="delivery">Dostawa</option>
          </select>
          <button className="h-10 rounded-md border bg-white px-3 text-sm text-slate-900" onClick={() => setSortOrder((o) => (o === "desc" ? "asc" : "desc"))}>
