...
import {
  Home, ShoppingCart, Clock, List, Calendar, Settings, LogOut,
  ChevronLeft, ChevronRight, Star, Utensils
} from "lucide-react";
...
function getCookie(name: string) {
  ...
  if (!m) return null;
  try { return decodeURIComponent(m.split("=")[1]); } catch { return m.split("=")[1]; }
}
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const ui: Role = r === "owner" || r === "manager" ? "admin" : "employee";
        if (live) setRole(ui);
      } catch {}
    };
  ...
  useEffect(() => {
    const ensure = async () => {
      ...
      try {
        const q = finalSlug ? `?restaurant=${encodeURIComponent(finalSlug)}` : "";
        const r = await fetch(`/api/restaurants/ensure-cookie${q}`, { cache: "no-store", credentials: "include" });
        const j = await r.json().catch(() => ({}));
        const s = j?.restaurant_slug || finalSlug || null;
        if (s && !urlSlug) {
          // dopnij slug do aktualnej ścieżki
          router.replace(`${pathname}?restaurant=${encodeURIComponent(s)}`);
        }
  ...
  const withQs = useMemo(
    () => (href: string) => (slug ? `${href}?restaurant=${encodeURIComponent(slug)}` : href),
    [slug]
  );
